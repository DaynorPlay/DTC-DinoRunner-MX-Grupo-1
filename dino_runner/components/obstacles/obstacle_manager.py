...
class ObstacleManager:
    ...
    def update(self, game):
        if len(self.obstacles)==0:
            cactus_size = random.randint(0,4)
            if cactus_size == 0:
        ...
        for obstacle in self.obstacles:
            ...
            if (game.player.dino_rect.colliderect(obstacle.rect) and not(game.player.shield) and not(game.player.hammer) and obstacle.name!="pill" and obstacle.name !="poison") or (game.player.dino_rect.colliderect(obstacle.rect) and game.player.hammer and obstacle.name == "bird"):
                ...
                self.obstacles.pop()
            elif game.player.dino_rect.colliderect(obstacle.rect) and obstacle.name == "pill" and game.player.pill==False and game.player.velocity ==False:
                ...
                self.obstacles.pop()
            elif game.player.dino_rect.colliderect(obstacle.rect) and obstacle.name == "poison" and game.player.poison == False and game.player.velocity ==False:
                obstacle.start_time = pygame.time.get_ticks()
